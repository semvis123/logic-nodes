<script lang="ts">
	...
	const DEFAULTS = { s: 'A ∩ (B ∪ C)', sets: 'auto', labels: 'none' };
	...
	$: syncUrl({ s: input, sets, labels }, DEFAULTS);
	...
	function saveSvg() {
		downloadSvg(vennSvg(result.n, result.shaded, { labels, title: result.text }), `${fileName(result.text)}.svg`);
	}

	function savePng() {
		downloadPng(vennSvg(result.n, result.shaded, { labels, title: result.text }), `${fileName(result.text)}.png`);
	}
	...
	const page = {
		...
		description:
			'Type a set expression such as (A ∪ B)′ ∩ C and see its Venn diagram shaded, or click regions to get the expression. Up to three sets, with truth tables.',
...
>
	<section class="intro">
		...
		<p class="lede">
			...
			click the regions you want shaded and get the expression back.
		</p>
					...
					<tbody>
						{#each result.regions as region}
							<tr class:on={region.shaded}>
								...
								<td class="words">{region.words}</td>
	...
	<section id="karnaugh">
		...
		<p class="reducer">
			The labels match: region m5 of the diagram is cell m5 of the map. Past three sets circles run out of room, and a
			map keeps going: the <a href="/karnaugh-map-solver">Karnaugh map solver</a> handles up to six variables.
		</p>
	</section>
